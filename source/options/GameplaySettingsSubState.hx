...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		*/
		...
		var option:Option = new Option('Auto-Reset on Fail',
			"If checked, failing a song will instantly restart it.",
			'autoReset',
			'bool',
			false);
		addOption(option);
		...
		var option:Option = new Option('Hit Window Type',
			...
			'string',
			'Standard*',
			['Peaceful', 'Lenient', 'Chill', 'Standard*', 'Tough', 'Extreme', 'Impossible', 'JUSTICE']); //yes, fkin 8 presets.
		addOption(option);
